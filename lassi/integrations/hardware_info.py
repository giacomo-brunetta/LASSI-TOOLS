...
# ---------------------------------------------------------------------------
# get_gpu_info_impl  (unchanged behavior)
# ---------------------------------------------------------------------------
...
async def get_gpu_info_impl() -> str:
    ...
    # macOS / Apple Silicon: no SMI binary, but system_profiler enumerates GPUs.
    if platform.system() == "Darwin" and shutil.which("system_profiler"):
        output = await run_cmd(["system_profiler", "-json", "SPDisplaysDataType"])
        if not output.startswith("Error") and not output.startswith("Execution failed"):
            try:
                data = json.loads(output)
                lines: list[str] = ["Apple GPU Detected:"]
                for entry in data.get("SPDisplaysDataType", []):
                    name = entry.get("sppci_model") or entry.get("_name") or "Apple GPU"
                    vendor = entry.get("spdisplays_vendor") or entry.get("sppci_vendor") or "Apple"
                    cores = entry.get("sppci_cores") or "?"
                    metal = entry.get("spdisplays_mtlgpufamilysupport") or entry.get("spdisplays_metal") or "—"
                    lines.append(f"  {name} — vendor={vendor}, cores={cores}, metal={metal}")
                    for disp in entry.get("spdisplays_ndrvs", []) or []:
                        d_name = disp.get("_name") or "display"
                        res = disp.get("_spdisplays_resolution") or disp.get("spdisplays_resolution") or "?"
                        lines.append(f"    display: {d_name} @ {res}")
                return "\n".join(lines)
            except (json.JSONDecodeError, AttributeError):
                pass

    return (
        "No GPU enumeration tool found "
        "(nvidia-smi, rocm-smi, xpu-smi, system_profiler) — host has no dedicated GPU or its driver tools are not installed."
    )
