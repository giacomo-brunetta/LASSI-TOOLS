...
3. Call `get_toolchain_info` and detect/record active toolchain versions from that MCP result before implementation:
   - `torch` version
...
- Include in `LASSI/translation_notes.md`:
  ...
  - detected `torch`, `torch-mlir`, Python, and LLVM versions from `get_toolchain_info`
