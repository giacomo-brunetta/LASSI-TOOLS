...
6. **Phase 4: Model Generation**
   ...
   - Require this exact artifact generation sequence unless the MCP tool path fails with concrete error evidence:
     1) call `export_model_to_pt`
     2) confirm the `.pt` artifact exists and is non-empty
     3) call `compile_torch_to_mlir` on that `.pt` artifact
     4) confirm the `.mlir` artifact exists and is non-empty
   ...
   - Enforce this sequence from `LASSI/how_to_make_it_work.md`:
     ...
     4) Run export (`python export_model.py`) only after validation passes.
     5) Ensure `export_model.py` imports the selected variant explicitly rather than assuming a default module/class name.
   ...
   - Require Model Generator to read at minimum before export:
     ...
     - `export_model.py` if it already exists
...
8. Reject and reroute Phase 4 output when:
   ...
   - `export_model.py` is missing, incomplete, or not wired to the selected variant.
...
- Treat a missing `export_model.py`, missing selected-variant wiring, or inability to run the original C/C++ oracle as blocking failures, not soft warnings.
