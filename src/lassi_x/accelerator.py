...
from .validation import fixture_relative_path, validate_candidate
...
async def repair_accelerator_candidate(
    ...
    staged = await _stage_context(config, execution, slug)
...
"""
    ...
    try:
        ...
        for attempt in range(config.compatibility.correction_rounds + 1):
            ...
            diagnostic_dir = run_dir / "diagnostics" / "accelerator" / slug / f"attempt-{attempt}"
