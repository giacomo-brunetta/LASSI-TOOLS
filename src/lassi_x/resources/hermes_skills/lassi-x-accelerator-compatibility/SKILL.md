...
1. Identify every intended accelerator/compiler stack and precision. Do not substitute results
   from another vendor, compiler version, or the legacy Torch-MLIR/TOSA corpus.
...
1. Run `lassi-x-compat-wiki targets` and select the exact published target. Current target IDs are
   `graphcore-pod64-poptorch`, `alcf-cs3-cerebras-pytorch`, and `groq-r01-groqflow`.
2. Inventory the actual `aten.*` operators produced or expected by the candidate. If a scripted
   model is available, use `lassi-x-compat validate MODEL_PATH` to discover its operator set.
