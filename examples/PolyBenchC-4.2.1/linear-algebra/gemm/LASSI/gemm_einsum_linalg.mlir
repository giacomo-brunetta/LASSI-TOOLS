...
module attributes {torch.debug_module_name = "GemmEinsum"} {
  ...
  func.func @forward(%arg0: tensor<1000x1100xf32>, %arg1: tensor<1000x1200xf32>, %arg2: tensor<1200x1100xf32>) -> tensor<1000x1100xf32> {
    ...
    %2 = tensor.empty() : tensor<1x1000x1100xf32>
    %3 = linalg.fill ins(%cst_0 : f32) outs(%2 : tensor<1x1000x1100xf32>) -> tensor<1x1000x1100xf32>
    %4 = linalg.batch_matmul ins(%expanded, %expanded_2 : tensor<1x1000x1200xf32>, tensor<1x1200x1100xf32>) outs(%3 : tensor<1x1000x1100xf32>) -> tensor<1x1000x1100xf32>
    %collapsed = tensor.collapse_shape %4 [[0, 1], [2]] : tensor<1x1000x1100xf32> into tensor<1000x1100xf32>
    %5 = linalg.generic {indexing_maps = [#map, #map1, #map1], iterator_types = ["parallel", "parallel"]} ins(%cst_1, %collapsed : tensor<f32>, tensor<1000x1100xf32>) outs(%0 : tensor<1000x1100xf32>) {
    ^bb0(%in: f32, %in_3: f32, %out: f32):
      %7 = arith.mulf %in, %in_3 : f32
      linalg.yield %7 : f32
    } -> tensor<1000x1100xf32>
    %6 = linalg.generic {indexing_maps = [#map1, #map1, #map1], iterator_types = ["parallel", "parallel"]} ins(%1, %5 : tensor<1000x1100xf32>, tensor<1000x1100xf32>) outs(%0 : tensor<1000x1100xf32>) {
    ^bb0(%in: f32, %in_3: f32, %out: f32):
