...
class BackendConfig(StrictModel):
    """Settings shared by every measurement backend."""
...
class TorchBackendConfig(BackendConfig):
    """PyTorch eager backend on a CPU, CUDA device, or similar device string."""
    ...
    @classmethod
    def require_device(cls, value: object) -> object:
        if isinstance(value, dict) and not value.get("device"):
            raise ValueError("torch backend requires device")
        return value


class GroqBackendConfig(BackendConfig):
    """Groq backend using a shared queue, PBS login resource, or direct node."""
    ...
    @model_validator(mode="after")
    def backend_requirements(self) -> GroqBackendConfig:
...
class NativeBackendConfig(BackendConfig):
    """Native Graphcore or Cerebras worker configuration."""

    type: Literal["native"]
    architecture: Literal["graphcore_ipu", "cerebras_wse3"]
    worker: NativeWorkerConfig
    ...
    @model_validator(mode="after")
    def backend_requirements(self) -> NativeBackendConfig:
